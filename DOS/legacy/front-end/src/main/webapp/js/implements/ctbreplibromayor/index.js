$(function() {
	var config =  {
		...
		tituloApp: 'Reporte de Libro Mayor' ,                 
		...
		getController: function(){
			return this.contextpath + "/controllers/ctbreplibromayor";
	...
	$genera_PDF.click(function(event){
		...
		var input_json = config.getUrlForGetAndPost() + '/getPdfLibroMayor/'+cadena+'/'+iu+'/out.json';
	...
	$busqueda_reporte.click(function(event){
		...
		$.post(restful_json_service,arreglo_parametros,function(entry){
			...
			html_fila_vacia +='<td align="left"  id="sin_borde" width="135px" height="10"></td>';
			...
			if(parseInt(body_tabla.length)>0){
				...
				for(var i=0; i<body_tabla.length; i++){
					...
					html_reporte +='<td align="left">'+body_tabla[i]["periodo"]+'</td>';
					html_reporte +='<td align="right">'+$(this).agregar_comas(body_tabla[i]["cargos"])+'</td>';
					html_reporte +='<td align="right">'+$(this).agregar_comas(body_tabla[i]["abonos"])+'</td>';
					html_reporte +='<td align="right">'+$(this).agregar_comas(body_tabla[i]["saldo"])+'</td>';
					html_reporte +='<td align="right">'+$(this).agregar_comas(body_tabla[i]["cargos1"])+'</td>';
					html_reporte +='<td align="right">'+$(this).agregar_comas(body_tabla[i]["abonos1"])+'</td>';
					html_reporte +='</tr>';
				}
