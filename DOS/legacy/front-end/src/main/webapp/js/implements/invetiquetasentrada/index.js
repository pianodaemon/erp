$(function() {
	...
	var controller = $contextpath.val()+"/controllers/invetiquetasentrada";
	...
	$tabs_li_funxionalidad = function(){
		var $select_prod_tipo = $('#forma-invetiquetasentrada-window').find('select[name=prodtipo]');
		$('#forma-invetiquetasentrada-window').find('#submit').mouseover(function(){
			$('#forma-invetiquetasentrada-window').find('#submit').removeAttr("src").attr("src","../../img/modalbox/bt1.png");
		});
		$('#forma-invetiquetasentrada-window').find('#submit').mouseout(function(){
			$('#forma-invetiquetasentrada-window').find('#submit').removeAttr("src").attr("src","../../img/modalbox/btn1.png");
		});
		
		$('#forma-invetiquetasentrada-window').find('#boton_cancelar').mouseover(function(){
			$('#forma-invetiquetasentrada-window').find('#boton_cancelar').css({backgroundImage:"url(../../img/modalbox/bt2.png)"});
		});
		$('#forma-invetiquetasentrada-window').find('#boton_cancelar').mouseout(function(){
			$('#forma-invetiquetasentrada-window').find('#boton_cancelar').css({backgroundImage:"url(../../img/modalbox/btn2.png)"});
		});
		
		$('#forma-invetiquetasentrada-window').find('#close').mouseover(function(){
			$('#forma-invetiquetasentrada-window').find('#close').css({backgroundImage:"url(../../img/modalbox/close_over.png)"});
		});
		$('#forma-invetiquetasentrada-window').find('#close').mouseout(function(){
			$('#forma-invetiquetasentrada-window').find('#close').css({backgroundImage:"url(../../img/modalbox/close.png)"});
		});
		
		$('#forma-invetiquetasentrada-window').find(".contenidoPes").hide(); //Hide all content
		$('#forma-invetiquetasentrada-window').find("ul.pestanas li:first").addClass("active").show(); //Activate first tab
		$('#forma-invetiquetasentrada-window').find(".contenidoPes:first").show(); //Show first tab content
		
		//On Click Event
		$('#forma-invetiquetasentrada-window').find("ul.pestanas li").click(function() {
			$('#forma-invetiquetasentrada-window').find(".contenidoPes").hide();
			$('#forma-invetiquetasentrada-window').find("ul.pestanas li").removeClass("active");
			var activeTab = $(this).find("a").attr("href");
			$('#forma-invetiquetasentrada-window').find( activeTab , "ul.pestanas li" ).fadeIn().show();
	...
	$calcula_totales = function(){
		//var $arreglo_grid = $('#forma-invetiquetasentrada-window').find('input[name=arreglo]');
		var $campo_flete = $('#forma-invetiquetasentrada-window').find('input[name=flete]');
		var $monto_iva_flete = $('#forma-invetiquetasentrada-window').find('input[name=iva_flete]');
		
		var $tasa_fletes = $('#forma-invetiquetasentrada-window').find('input[name=tasafletes]');
		var $campo_subtotal = $('#forma-invetiquetasentrada-window').find('input[name=subtotal]');
		//var $campo_descuento = $('#forma-invetiquetasentrada-window').find('input[name=descuento]');
		var $campo_impuesto = $('#forma-invetiquetasentrada-window').find('input[name=totimpuesto]');
		var $retencion = $('#forma-invetiquetasentrada-window').find('input[name=retencion]');
		var $campo_total = $('#forma-invetiquetasentrada-window').find('input[name=total]');
		var sumaImporte = 0;
		//var sumaDescuento = 0;
		var sumaImpuesto = 0;
		var sumaTotal = 0;
		...
		var $grid_productos = $('#forma-invetiquetasentrada-window').find('#grid_productos');
	...
	{	
		...
		$.post(input_json,$arreglo,function(valor){
			$('#forma-invetiquetasentrada-window').find('input[name=tc]').val(valor['tipoCambio'][0]['valor']);
		},"json");
	}
	...
	var carga_formainvetiquetasentrada00_for_datagrid00 = function(id_to_show, accion_mode){
		...
		if(accion_mode == 'cancel'){
			...
			var form_to_show = 'formainvetiquetasentrada00';
			$('#' + form_to_show).each (function(){   this.reset(); });
			var $forma_selected = $('#' + form_to_show).clone();
			$forma_selected.attr({ id : form_to_show + id_to_show });
			//var accion = "get_datos_entrada_mercancia";
			
			$(this).modalPanel_invetiquetasentrada();
			
			$('#forma-invetiquetasentrada-window').css({ "margin-left": -425, 	"margin-top": -230 });
			
			$forma_selected.prependTo('#forma-invetiquetasentrada-window');
			...
			if(accion_mode == 'edit'){
				...
				var input_json = document.location.protocol + '//' + document.location.host + '/'+controller+'/getEntrada.json';
				$arreglo = {'identificador':id_to_show,
							'iu':$('#lienzo_recalculable').find('input[name=iu]').val()
							};
				
				var $identificador = $('#forma-invetiquetasentrada-window').find('input[name=identificador]');
				var $folio_entrada = $('#forma-invetiquetasentrada-window').find('input[name=folio_entrada]');
				var $estatus = $('#forma-invetiquetasentrada-window').find('input[name=estatus]');
				var $accion = $('#forma-invetiquetasentrada-window').find('input[name=accion]');
				var $select_tipo_movimiento = $('#forma-invetiquetasentrada-window').find('select[name=select_tipo_movimiento]');
				var $tasa_fletes = $('#forma-invetiquetasentrada-window').find('input[name=tasafletes]');
				var $total_tr = $('#forma-invetiquetasentrada-window').find('input[name=total_tr]');
				var $campo_factura = $('#forma-invetiquetasentrada-window').find('input[name=factura]');
				var $campo_ordencompra = $('#forma-invetiquetasentrada-window').find('input[name=ordencompra]');
				var $campo_numeroguia = $('#forma-invetiquetasentrada-window').find('input[name=numeroguia]');
				var $campo_expedicion = $('#forma-invetiquetasentrada-window').find('input[name=expedicion]');
				var $select_denominacion = $('#forma-invetiquetasentrada-window').find('select[name=denominacion]');
				var $select_fleteras = $('#forma-invetiquetasentrada-window').find('select[name=fletera]');
				var $select_almacen_destino = $('#forma-invetiquetasentrada-window').find('select[name=almacen_destino]');
				var $campo_tc = $('#forma-invetiquetasentrada-window').find('input[name=tc]');
				var $campo_observaciones = $('#forma-invetiquetasentrada-window').find('textarea[name=observaciones]');
				var $select_tipo_doc = $('#forma-invetiquetasentrada-window').find('select[name=tipodoc]');
				
				//campos del proveedor
				var $hidden_id_proveedor = $('#forma-invetiquetasentrada-window').find('input[name=id_proveedor]');
				var $campo_rfc_proveedor = $('#forma-invetiquetasentrada-window').find('input[name=rfcproveedor]');
				var $campo_razon_proveedor = $('#forma-invetiquetasentrada-window').find('input[name=razon_proveedor]');
				//var $campo_dir_proveedor = $('#forma-invetiquetasentrada-window').find('input[name=dir_proveedor]');
				var $campo_tipo_proveedor = $('#forma-invetiquetasentrada-window').find('input[name=tipo_proveedor]');
				var $campo_sku = $('#forma-invetiquetasentrada-window').find('input[name=sku_producto]');
				...
				var $campo_impuesto = $('#forma-invetiquetasentrada-window').find('input[name=totimpuesto]');
				var $campo_total = $('#forma-invetiquetasentrada-window').find('input[name=total]');
				var $campo_ieps = $('#forma-invetiquetasentrada-window').find('input[name=ieps]');
				
				var $etiqueta_flete = $('#forma-invetiquetasentrada-window').find('#etiqueta_flete');
				var $etiqueta_ieps = $('#forma-invetiquetasentrada-window').find('#etiqueta_ieps');
				var $etiqueta_ret = $('#forma-invetiquetasentrada-window').find('#etiqueta_ret');
				
				
				//href para buscar producto
				var $buscar_producto = $('#forma-invetiquetasentrada-window').find('a[href*=busca_producto]');
				//href para agregar producto al grid
				var $agregar_producto = $('#forma-invetiquetasentrada-window').find('a[href*=agregar_producto]');
				
				var $cancelar = $('#forma-invetiquetasentrada-window').find('#cancelar');
				var $descargar_pdf = $('#forma-invetiquetasentrada-window').find('#descargar_pdf');
				
				var $cerrar_plugin = $('#forma-invetiquetasentrada-window').find('#close');
				var $cancelar_plugin = $('#forma-invetiquetasentrada-window').find('#boton_cancelar');
				var $submit_actualizar = $('#forma-invetiquetasentrada-window').find('#submit');
				...
				var respuestaProcesada = function(data){
					if ( data['success'] == "true" ){
						...
						$('#forma-invetiquetasentrada-overlay').fadeOut(remove);
						$get_datos_grid();
					}else{
						//habilitar boton actualizar
						$submit_actualizar.removeAttr('disabled');
						// Desaparece todas las interrogaciones si es que existen
						$('#forma-invetiquetasentrada-window').find('div.interrogacion').css({'display':'none'});
						...
						$('#forma-invetiquetasentrada-window').find('#div_warning_grid').css({'display':'none'});
						$('#forma-invetiquetasentrada-window').find('#div_warning_grid').find('#grid_warning').children().remove();
						...
						for (var element in valor){
							...
							if( longitud.length > 1 ){
								$('#forma-invetiquetasentrada-window').find('img[rel=warning_' + tmp.split(':')[0] + ']')
								.parent()
								.css({'display':'block'})
								...
								$('#forma-invetiquetasentrada-window').find('.invetiquetasentrada_div_one').css({'height':'585px'});
								$('#forma-invetiquetasentrada-window').find('#div_warning_grid').css({'display':'block'});
								...
								$('#forma-invetiquetasentrada-window').find('#div_warning_grid').find('#grid_warning').append(tr_warning);
								
							}
						}
						$('#forma-invetiquetasentrada-window').find('#div_warning_grid').find('#grid_warning').find('tr:odd').find('td').css({ 'background-color' : '#FFFFFF'});
						$('#forma-invetiquetasentrada-window').find('#div_warning_grid').find('#grid_warning').find('tr:even').find('td').css({ 'background-color' : '#e7e8ea'});
					}
				}
				...
				$.post(input_json,$arreglo,function(entry){
					...
					if(parseInt(countDisplay)==1){
						$('#forma-invetiquetasentrada-window').find('.invetiquetasentrada_div_one').css({'height':'555px'});
					}
					
					if(parseInt(countDisplay)==2){
						$('#forma-invetiquetasentrada-window').find('.invetiquetasentrada_div_one').css({'height':'570px'});
					}
				...
				$descargar_pdf.click(function(event){
					...
					var input_json = document.location.protocol + '//' + document.location.host + '/' + controller + '/get_genera_pdf_invetiquetasentrada/'+$identificador.val()+'/'+iu+'/out.json';
	...
	$get_datos_grid = function(){
		...
		$.post(input_json,$arreglo,function(data){
			//pinta_grid
			//$.fn.tablaOrdenable(data,$('#lienzo_recalculable').find('.tablesorter'),carga_formainvetiquetasentrada00_for_datagrid00);
			
			//aqui se utiliza el mismo datagrid que prefacturas. Solo muesta icono de detalles, el de eliminar No
			$.fn.tablaOrdenablePrefacturas(data,$('#lienzo_recalculable').find('.tablesorter'),carga_formainvetiquetasentrada00_for_datagrid00);
