$(function() {
          var config =  {
                    tituloApp: 'Anticipos no Aplicados' ,                 
                    ...
                    getController: function(){
                        return this.contextpath + "/controllers/repanticiposnoaplicados";
          ...
          var $Buscar_anticiposnoaplicados= $('#lienzo_recalculable').find('div.repcobranzadiaria').find('table#fechas tr td').find('input[value$=Buscar]');
          ...
          $Buscar_anticiposnoaplicados.click(function(event){
               event.preventDefault();
               ...
               ///GENERA LA VISTA DE LA IMPRESION DEL REPORTE DE POR CLIENTE
                    ...
                    if($fecha_inicial.val() != "" && $fecha_final.val() != ""){ 
                         ...
                         var restful_json_service = config.getUrlForGetAndPost() + '/getAnticiposnoAplicados/out.json';
                         ...
                         var producto="";
                            ...
                            var html_anticipos = '<table id="ventas_diarias">';
                            var porcentaje = 0.0;
                            ...
                            html_anticipos +='<thead> <tr>';

                                for(var key in header_tabla){
                                    var attrValue = header_tabla[key];
                                     
                                    if(attrValue == "Fecha"){
                                        html_anticipos +='<td  align="CENTER" width="90px" >'+attrValue+'</td>'; 
                                    }
                                    
                                    if(attrValue == "Cliente"){
                                        html_anticipos +='<td  align="LEFT"  width="450px" >'+attrValue+'</td>'; 
                                    }

                                   
                                    if(attrValue == "Anticipos "){
                                        html_anticipos +='<td  align="center" width="110px">'+attrValue+'</td>'; 
                                    }
                                    
                                    if(attrValue == "Observaciones"){
                                        html_anticipos +='<td  align="center" width="290px" >'+attrValue+'</td>'; 
                                    }    
                                }

                              html_anticipos +='</tr> </thead>';
								
                              for(var i=0; i<body_tabla.length; i++){
                                html_anticipos +='<tr>';
                                    //html_anticipos +='<td align="center"  width="100px">'+body_tabla[i]["factura"]+'</td>'; 
                                    html_anticipos +='<td align="center" width="90px">'+body_tabla[i]["fecha_anticipo"]+'</td>'; 
                                    html_anticipos +='<td align="left"  width="450px">'+body_tabla[i]["cliente"]+'</td>'; 
                                    html_anticipos +='<td align="right"  width="110px">$&nbsp;&nbsp;'+$(this).agregar_comas(parseFloat(body_tabla[i]["anticipo_actual"]).toFixed(2))+'</td>'; 
                                    html_anticipos +='<td align="left"  width="290px">'+body_tabla[i]["observaciones"]+'</td>'; 
                                    
                                html_anticipos +='</tr>';
                              }
                              
                        html_anticipos += '</table>';
								
                        $div_cobranzadiaria.append(html_anticipos); 
                    ...
                    //click generar reporte de pronostico de anticipos no aplicados
          $genera_reporte_cobranzadiaria.click(function(event){
            ...
            if(fecha_inicial != 0 && fecha_final !=0){
                var input_json = config.getUrlForGetAndPost() + '/getAnticiposnoAplicados/'+cadena+'/out.json';
