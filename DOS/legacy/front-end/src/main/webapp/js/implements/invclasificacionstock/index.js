$(function() {
	...
	var controller = $contextpath.val()+"/controllers/invclasificacionstock";
                                                          
   
    //Barra para las acciones
    $('#barra_acciones').append($('#lienzo_recalculable').find('.table_acciones'));
    $('#barra_acciones').find('.table_acciones').css({'display':'block'});
	var $new_Stock = $('#barra_acciones').find('.table_acciones').find('a[href*=new_item]');
	...
	$('#barra_titulo').find('#td_titulo').append('Cat&aacute;logo de Clasificacion Stock');
	...
	var to_make_one_search_string = function(){
		...
		var valor_retorno = "";
		var signo_separador = "=";
		valor_retorno += "ClasificacionStock" + signo_separador + $busqueda_clasificacion.val() + "|";
	...
	//nuevo stock
	$new_Stock.click(function(event){
		event.preventDefault();
		var id_to_show = 0;
		$(this).modalPanel_Stock();   //llamada al plug in 
		...
		var respuestaProcesada = function(data){
			if ( data['success'] == "true" ){
				jAlert("La Clasifcacion&nbsp; fue dada de alta con exito", 'Atencion!');
		...
		var input_json = document.location.protocol + '//' + document.location.host + '/'+controller+'/getStock.json';
...
//		});
	...
	var carga_formaCC00_for_datagrid00 = function(id_to_show, accion_mode){
		...
		if(accion_mode == 'cancel'){
			...
			jConfirm('Realmente desea eliminar la Clasifcacion&nbsp; seleccionada', 'Dialogo de confirmacion', function(r) {
				if (r){
					$.post(input_json,$arreglo,function(entry){
						if ( entry['success'] == '1' ){
							jAlert("La Stock fue eliminado exitosamente", 'Atencion!');
							$get_datos_grid();
						}
						else{
							jAlert("La Clasifcacion&nbsp; no pudo ser eliminado.", 'Atencion!');
						}
					},"json");
				}
			});
			...
			if(accion_mode == 'edit'){
                            ...
                            //aqui es el post que envia los datos a getStock.json  cuando se va a editar
				var input_json = document.location.protocol + '//' + document.location.host + '/'+controller+'/getStock.json';
				$arreglo = {'id':id_to_show,
							'iu': $('#lienzo_recalculable').find('input[name=iu]').val()
				};
				...
				$.post(input_json,$arreglo,function(entry){
					$campo_id.attr({'value' : entry['Stock']['0']['id']});
					$clasificacion.attr({'value' : entry['Stock']['0']['clasificacion']});
					$descripcion.attr({'value' : entry['Stock']['0']['descripcion']});			
				},"json");//termina llamada json
    ...
    $get_datos_grid = function(){
        var input_json = document.location.protocol + '//' + document.location.host + '/'+controller+'/getAllClasificacionstock.json';
        
        var iu = $('#lienzo_recalculable').find('input[name=iu]').val();
        
        $arreglo = {'orderby':'id','desc':'DESC','items_por_pag':10,'pag_start':1,'display_pag':10,'input_json':'/'+controller+'/getAllClasificacionstock.json', 'cadena_busqueda':$cadena_busqueda, 'iu':iu}
