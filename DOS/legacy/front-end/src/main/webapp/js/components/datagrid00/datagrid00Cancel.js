$.fn.tablaOrdenableCancel = function(data,grid,loadForm){
	...
	$clicktr = function(item, $tbodygrid){
		...
		//$tr_clickeado = $tbodygrid.find('a[href="editar_'+item+'"],a[href="cancelar_'+item+'"]');
		$tr_clickeado = $tbodygrid.find('input#cancel_'+item);
		...
		for ( iterador = 0; iterador < items.length; iterador++ ){
			...
			$theadgrid.find('tr th').each(function(i){
				...
				if($item_header == 'id'){
					...
					//$tr += '<td><INPUT TYPE="button" classs="cancel" id="cancel_'+ items[iterador][$item_header]+'" value="Cancelar" style="width:65px; font-weight: bold;"></td>';
					$tr += items[iterador]['accion'];
					$id_item = items[iterador][$item_header];
				}else{
